...
void ofApp::update() {
	...
	for (int i = 0; i < 30; i++) {
		...
		color.setHsb(ofMap(i, 0, 30, 0, 255), 200, 230);
	...
	for (int i = 0; i < this->log_list.size(); i++) {
		...
		auto next = this->log_list[i].back() + glm::normalize(future) * 1;
		next = glm::length(next) < 320 ? next : glm::normalize(next) * 320;
...
void ofApp::draw() {
	...
	ofNoFill();
	ofSetColor(128);
	ofDrawCircle(glm::vec2(), 255);

	ofFill();
	for (int i = 0; i < this->log_list.size(); i++) {

		ofSetColor(this->color_list[i]);
		auto size = this->log_list[i].size() < 3 ? 10 : ofMap(this->log_list[i].size(), 3, 20, 10, 0);
	...
	for (int i = 0; i < this->log_list.size(); i++) {
		...
		ofSetColor(39);
		auto size = this->log_list[i].size() < 3 ? 5 : ofMap(this->log_list[i].size(), 3, 20, 5, 0);
		ofDrawCircle(this->log_list[i].back(), size);
	}
